...
Validates:
- build_train_predict_fn produces a valid callable
- _train_and_predict works for each model type
- verify_window_with_ev correctly identifies winners and ties
- run_training_window_optimization orchestrates end-to-end
...
class TestCleanFeatures:
    def test_replaces_inf(self):
        ...
        y = np.array([0, 1])
        X_clean, y_clean = _clean_features(X, y)
    ...
    def test_keeps_partial_nan_rows(self):
        ...
        y = np.array([0, 1])
        X_clean, y_clean = _clean_features(X, y)
        assert len(y_clean) == 2  # Partial NaN rows are kept
...
class TestVerifyWindowWithEV:
    def test_clear_winner_verified(self, mock_window_report):
        """When there's a clear Brier winner, it should be verified."""
    ...
    def test_tied_windows_prefer_shorter(self, tied_window_report):
        ...
        assert ev.brier_delta < 0.005  # Tied
        # all_available (None=999) is longer than last_5 (5)
        # So ev_verified should be False (best is longer, not shorter)
        assert ev.ev_verified is False
...
class TestOptimizerIntegration:
    ...
    def test_ev_verification_on_optimizer_output(self):
        """EV verification should work on actual optimizer output."""
        def mock_train_predict(train_data, eval_data, model_type):
            n = len(train_data)
            brier = 0.25 - 0.005 * min(n, 10)
            return (brier, None, None)
