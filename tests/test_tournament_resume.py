...
class TestTournamentResumeComposite:
    ...
    def test_strong_team_scores_high(self):
        ...
        score = compute_tournament_resume_composite(
            q1_win_pct=0.75,
            q1_games=12,
            road_neutral_win_pct=0.80,
            road_neutral_games=16,
            elite_sos=22.0,
            sor=1.3,
        )
    ...
    def test_weak_team_scores_low(self):
        ...
        score = compute_tournament_resume_composite(
            q1_win_pct=0.10,
            q1_games=3,
            road_neutral_win_pct=0.35,
            road_neutral_games=12,
            elite_sos=5.0,
            sor=0.5,
        )
    ...
    def test_output_range_zero_to_one(self):
        ...
        high = compute_tournament_resume_composite(
            q1_win_pct=1.0,
            q1_games=20,
            road_neutral_win_pct=1.0,
            road_neutral_games=20,
            elite_sos=30.0,
            sor=2.0,
        )
        assert 0.0 <= high <= 1.0
    ...
    def test_strong_beats_mid_beats_weak(self):
        ...
        mid = compute_tournament_resume_composite(
            q1_win_pct=0.40,
            q1_games=8,
            road_neutral_win_pct=0.60,
            road_neutral_games=14,
            elite_sos=14.0,
            sor=0.95,
        )
        ...
        assert strong > mid > weak, f"Ordinality violated: strong={strong:.4f}, mid={mid:.4f}, weak={weak:.4f}"
...
class TestBayesianShrinkage:
    ...
    def test_more_games_less_shrinkage(self):
        ...
        many_games = compute_tournament_resume_composite(
            ...
            q1_games=15,
            road_neutral_win_pct=0.80,
            road_neutral_games=18,
            elite_sos=18.0,
            sor=1.0,
        )
        # Many games should produce a higher score (observed 80% > prior 40%)
        assert many_games > few_games, f"More games ({many_games:.4f}) should score higher than fewer ({few_games:.4f})"
...
class TestSORNormalization:
    ...
    def test_sor_one_is_average(self):
        ...
        baseline = compute_tournament_resume_composite(
            q1_win_pct=0.40,
            q1_games=10,
            road_neutral_win_pct=0.50,
            road_neutral_games=15,
            elite_sos=18.0,
            sor=1.0,
        )
...
class TestMathematicalProperties:
    ...
    def test_no_nan_or_inf(self):
        ...
        edge_cases = [
            dict(q1_win_pct=0.0, q1_games=0, road_neutral_win_pct=0.0, road_neutral_games=0, elite_sos=0.0, sor=0.0),
            dict(
                q1_win_pct=1.0, q1_games=100, road_neutral_win_pct=1.0, road_neutral_games=100, elite_sos=50.0, sor=5.0
            ),
            dict(q1_win_pct=0.0, q1_games=1, road_neutral_win_pct=0.0, road_neutral_games=1, elite_sos=-10.0, sor=-1.0),
        ]
