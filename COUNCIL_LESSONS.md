...
| **O2** | Validate eight local four-factor features against Torvik | Per-season, per-feature r ≥ 0.99, no systematic residual bias by team type / conference / tempo. Prereq: confirm Torvik docs state whether values are raw or opponent-adjusted. | 21 | **`open, blocked on O2a`** — 2026-04-13 fresh run of `scripts/validate_four_factors.py` exposed a **resolver-collision bug** in `compute_boxscore_ff` (fuzzy-containment matches were letting a 1-game D3 affiliate overwrite the real D1 entry when both resolved to the same canonical id; Vermont Catamounts' 35-game eFG=0.540 was being clobbered by `vermont_state_lyndon_hornets`' 1-game eFG=0.318). Fix applied (sort by game count desc + restrict resolver to `exact_id`/`prefix_strip` methods + first-writer-wins) jumped 4 of 8 features from mean r ≈ 0.45-0.63 to 0.95-0.97 and fixed the summary-print format-string crash. Remaining 4 features (TO%, ORB%, Opp TO%, DRB%) still fail the gate with a systematic bias traced to **undercounted turnovers/rebounds in the raw `historical_games_*.json`** — Vermont 2024 has 8.6 TO/game in source, Torvik implies ~15. See `artifacts/o2a_diagnostic_2026-04-13.md`. |
| **O2a** | Incomplete box-score fields in `historical_games_*.json` cause systematic -3% bias on TO%/Opp TO% and -1% on ORB%, +2% on DRB% | Identify which upstream provider(s) produce sparse box-score rows (missing `turnovers`, `orb`, `drb` fields or zero-valued for non-DI opponents). Either patch ingestion to pull complete box scores or redefine local FF as "complete-box-score-only" and filter. Then re-run validate_four_factors and verify r ≥ 0.99 per season per feature. | surfaced by O2 fresh validation 2026-04-13; root cause #1 (resolver collision) fixed same day | open |
...
| 31 | 2026-04-13 | Close O2: Four-Factor validation? | **NOT CLOSED; substantial progress.** Fresh run of `scripts/validate_four_factors.py` exposed two independent defects. **Root cause #1 (resolver collision, FIXED this commit):** `compute_boxscore_ff` was letting `TeamNameResolver`'s fuzzy `containment` method (conf 0.9) collapse unrelated teams — `vermont_state_lyndon_hornets` (1 game, eFG=0.318) was overwriting `vermont_catamounts` (35 games, eFG=0.540) because both resolved to canonical `vermont`. Same pattern for `tennessee_wesleyan_bulldogs` → `tennessee`. Fix: iterate raw tids in descending game-count order, restrict resolver to `exact_id`/`prefix_strip`, first-writer-wins. **Impact:** eFG%/FTR/Opp eFG%/Opp FTR went from mean r ≈ 0.45 to ≈ 0.96. **Root cause #2 (OPEN, spawned as O2a):** raw `historical_games_*.json` appears to systematically undercount turnovers and rebounds for affiliate/non-DI games. Vermont 2024 has 8.6 TO/game in the source; Torvik's published rate implies ~15. Produces consistent -3% bias on TO%/Opp TO%, -1% on ORB%, +2% on DRB%. Requires a source-data audit, not a code fix. Also fixed in same commit: `validate_four_factors.py:421` summary-print format-string crash (`{len(results):>5s}` on an int). *(§2 O2 still open, blocked on O2a)* |
