...
| **O2** | Validate eight local four-factor features against Torvik | Per-season, per-feature r ≥ 0.99, no systematic residual bias by team type / conference / tempo. Prereq: confirm Torvik docs state whether values are raw or opponent-adjusted. | 21 | **`open, blocked on O2a`** — 2026-04-13 fresh run of `scripts/validate_four_factors.py` against the post-O16-fix data shows **catastrophic FAIL** across all 17 production years × all 8 features. r ranges from 0.11 to 0.83; gate requires ≥ 0.99. Prereq (raw vs adjusted) was confirmed by code inspection: both sides are raw Oliver/Kubatko, so the divergence is NOT a definition mismatch. Root cause is a structural bug in `compute_boxscore_ff` / `ProprietaryMetricsEngine._four_factors`, surfaced by impossible local values for top seeds (Vermont local eFG% = 0.32, real ≈ 0.52). See `artifacts/o2a_diagnostic_2026-04-13.md`. Cannot close O2 until O2a is resolved. |
| **O2a** | Box-score adapter bug in `compute_boxscore_ff` produces impossible Four-Factor values for top tournament seeds | Identify and fix the per-game aggregation defect that gives Vermont/Florida/Illinois/Tennessee/Kansas (1- and 2-seeds, 2024) local eFG% in the 0.32–0.40 range when their published values are 0.51–0.54, with `local_efg + local_opp_efg ≈ 0.98–1.13`. Then re-run validate_four_factors and verify r ≥ 0.99 per season per feature. | surfaced by O2 fresh validation 2026-04-13 | open |
...
| 31 | 2026-04-13 | Close O2: Four-Factor validation? | **NOT CLOSED.** Fresh run of `scripts/validate_four_factors.py` against post-O16-fix data fails the gate catastrophically across all 17 years × 8 features (r ∈ [0.11, 0.83], gate requires ≥ 0.99). Inspection of 2024 1- and 2-seeds shows impossible local values (Vermont local eFG% = 0.32, real ≈ 0.52). Two hypotheses (raw-vs-adjusted, team1/team2 inversion) tested and falsified — root cause is a structural per-game aggregation bug in `compute_boxscore_ff` / `ProprietaryMetricsEngine._four_factors`. Spawned **O2a** with diagnostic at `artifacts/o2a_diagnostic_2026-04-13.md`. Per CLAUDE.md "Failure Recovery", stopped after two falsified hypotheses and escalated rather than continuing to dig. Council should prioritize a focused root-cause session on the box-score adapter before re-running any FF-dependent backtest. *(§2 O2 still open, blocked on O2a)* |
