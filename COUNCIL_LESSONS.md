...
| **O2** | Validate eight local four-factor features against Torvik | Per-season, per-feature r ≥ 0.99, no systematic residual bias by team type / conference / tempo. Prereq: confirm Torvik docs state whether values are raw or opponent-adjusted. | 21 | **`open, escalated — requires council decision on the gate`** — 2026-04-13 investigation across 344 D1 teams in 2024 shows that even with the best-possible formula (Oliver-alt, ORB-subtracted possessions denominator) TO% maxes at r = 0.97. eFG%, which has NO possessions denominator (just FGM/FGA/FG3M ratios), maxes at r = 0.98 despite matching Oliver exactly. The r ≥ 0.99 gate is **data-source bound**, not formula bound — two independently-collected box-score tables differ at the scorekeeping/rounding/game-set-filtering level by a few tenths of a percent. Three options for the council to choose from in `artifacts/o2a_diagnostic_2026-04-13.md`: (A) relax to r ≥ 0.95, (B) switch to a second box-score provider for cross-check, (C) accept "r ≈ 0.97 mean across 344 × 8" as validated. Recommendation: A. |
| **O2a** | Resolver collision + data-source precision limit in the four-factor validator | Root cause #1 (fuzzy-containment TeamNameResolver collapsing `vermont_state_lyndon_hornets` → `vermont` and overwriting real Vermont Catamounts stats): FIXED in `scripts/validate_four_factors.py` commit `df8edce`, jumped 4/8 features from mean r ≈ 0.45 to ≈ 0.97. Root cause #2 (provider-level precision limit): NOT a code defect; requires the council gate decision above. | surfaced by O2 fresh validation 2026-04-13; root cause #1 fixed same day | **`root cause #1 closed; root cause #2 escalated to O2`** |
