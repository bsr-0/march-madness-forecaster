...
def main() -> int:
    ...
    if args.scrape_results:
        ...
        ctx_path = default_dir / f"tournament_context_{args.year}.json"
        legacy_path = default_dir / f"tournament_results_{args.year}.json"
        results_source = str(ctx_path if ctx_path.exists() else legacy_path)
        if not actual_games:
            logger.error(
    ...
    if args.format == "kaggle":
        ...
        predictions = load_predictions_from_kaggle_csv(
            str(pred_path), mteams, known_ids=known_ids
        )
    else:
