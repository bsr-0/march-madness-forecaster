...
def build_results() -> List[GameResult]:
    ...
    """
    predictions_path = (
        Path(__file__).parent.parent / "data" / "processed" / "conference_tournament_predictions_2026.json"
    )
    ...
    def add_result(
        ...
        actual_loser = (
            team2_frag
            if actual_winner_frag.lower() in team1_frag.lower() or actual_winner_frag.lower() in pred["team1"].lower()
            else team1_frag
        )
    ...
    add_result(
        ...
        "Tennessee St.",
        "93-67",
        notes="Seed mismatch: model had different seedings than actual",
    )
    ...
    add_result("NEC", "NEC", "Championship", "LIU", "Mercyhurst", "LIU", "79-70")

    # Big South Championship (March 8): High Point (1) 91, Winthrop (2) 76
    add_result("BSth", "Big South", "Championship", "High Point", "Winthrop", "High Point", "91-76")

    # WCC Championship (March 10): Gonzaga (1) 79, Santa Clara (3) 68
    # Our model: Gonzaga vs Santa Clara — exact match!
    add_result("WCC", "WCC", "Round 6", "Gonzaga", "Santa Clara", "Gonzaga", "79-68")
    ...
    add_result("ACC", "ACC", "Play-in", "SMU", "Syracuse", "SMU", "86-69")
    ...
    add_result("B10", "Big Ten", "Round 1", "Northwestern", "Penn St.", "Northwestern", "76-66")
    ...
    add_result("B12", "Big 12", "Play-in", "Cincinnati", "Utah", "Cincinnati", "73-66")

    # BYU (10) 105, Kansas State (15) 91
    add_result("B12", "Big 12", "Play-in", "BYU", "Kansas St.", "BYU", "105-91")
    ...
    add_result(
        ...
        "Round 5",
        "Gonzaga",
        "Pacific",  # model had Gonzaga vs Pacific
        "Gonzaga",
        "65-56",
...
def evaluate(results: List[GameResult]):
    ...
    in_progress_confs = {"ACC", "Big Ten", "Big 12", "SEC", "Big East", "Mountain West"}
    ...
    if early_round_results:
        ...
        print(
            f"  IN-PROGRESS TOURNAMENTS (early rounds):  {ec}/{len(early_round_results)} picks correct ({ec / len(early_round_results):.1%})"
        )
    ...
    if low_conf_total > 0:
        print(
            f"  Low-confidence picks (<55%):   {low_conf_correct}/{low_conf_total} ({low_conf_correct / low_conf_total:.0%})"
        )
    ...
    if misses:
        ...
        for m in misses[:5]:
            print(
                f"    - {m.conference_name} {m.round_name}: predicted {m.predicted_winner} ({m.predicted_prob:.1%}), "
