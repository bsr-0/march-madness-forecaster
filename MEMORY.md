...
| D17 | Empirical-supremum ranker `P(b_i > max(actual pool opponents))` replacing IID-opponent P(rank=1) — O25 G2 | 4-yr × 50-candidate × 5000-MC backtest (2026-04-17) using the 18/25/32/30 real pool brackets per year in `pool_hist_results.json` | **Null across all 4 years.** Per-year Spearman ρ(new_ranker, actual_scores) = {2023: +0.371, 2024: −0.381, 2025: +0.274, 2026: −0.382} — oscillates near zero, never clears the 0.50 council gate. Winner-bracket rank under new ranker = {16, 32, 38, 41} — never top-5. Size-matched IID baseline (opponents drawn IID from pool marginals) performs equivalently at the 4-year window: ρ_iid = {+0.001, +0.426, −0.092, −0.489}; 2024 decisively favors IID. **O4 structural prediction partial**: var(pool_opp_max) > var(iid_opp_max) in 3/4 years consistent with pooled z = −4.15, but the consequent rank reordering is weak — top-10 improvers mean 7.50 R64 upsets vs bottom-10's 7.60 (essentially tied). **Root cause of null**: at n=4 years the within-portfolio ranking error is dominated by tournament-outcome uncertainty (the realized upset path determines which of the 50 stochastic candidates wins regardless of opponent structure), not by the opponent model's independence assumption. Keep the current IID-pool-marginals opponent ranker for the 2027 forecast. **Conditional dead-end**: reopen if (1) a future pool-year delivers ρ_new > 0.50 or winner-top-5 under this script, OR (2) the O4 lock flips positive (z ≥ 0), invalidating the independence assumption and making max(opponents) dispersion large enough to matter. Closes §2 O25 G2. | `COUNCIL_LESSONS.md §2 O25`; `scripts/o25_g2_empirical_supremum.py`; `artifacts/o25_g2_empirical_supremum_2026-04-17.json`; `tests/test_o25_g2_empirical_supremum_lock.py` |
