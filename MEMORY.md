...
| D17 | Empirical-supremum objective `P(b_i > max(actual pool opponents))` for within-portfolio bracket selection — O25 | 4-yr (2023-2026) × 50 `champ_first_tv` torvik candidates × 5000 MC × actual pool corpus | **Both rankers are noise against actual-outcome scores.** Mean Spearman ρ across 4 years: `p_sup` (new) = **+0.0605**, `p_rank1` (old) = **−0.0500**. G2-A gate required ρ > 0.50; the new ranker's 0.06 fails by an order of magnitude. G2-B (actual-#1 flips into top-5) fails in all 4 years. G2-C (O4 propagation sanity) fails in 2 of 4 years. The empirical-supremum objective is NOT a measurable improvement over P(rank=1) within a fresh torvik candidate portfolio under shape-encoded scoring. **Secondary finding (2026-04-16 this branch):** the original G1 preflight narrative in COUNCIL_LESSONS row 229 (spreads 0.835/0.315/0.045/0.310) was undocumented and unreproducible — no script, no artifact, and the numbers appear nowhere else in the repo. Real G1 re-run lands all 12 spread measurements (3 rankers × 4 years) in the AMBIGUOUS [0.016, 0.047] band; the pre-existing `rank_correlation_diagnostic.json` (production portfolio, n=15-17) shows similar magnitudes (0.028-0.092). G2 was therefore triggered on a preflight that didn't measurably support the trigger; the question now tested fresh and closed. Closes §2 O25. Conditional reactivate: if a better scoring function (team-identity rather than shape-encoded) produces materially different ρ, re-open. | `artifacts/o25_g1_real_2026-04-16.json`; `artifacts/o25_g2_empirical_supremum_2026-04-16.json`; `artifacts/o25_g3_diversity_2026-04-16.json`; `scripts/o25_g1_spread_diagnostic.py`; `scripts/o25_g2_empirical_supremum.py`; `tests/test_o25_g1_g2_lock.py` |
