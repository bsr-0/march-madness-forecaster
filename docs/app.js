...
const METRIC_EXPLAINER =
  'Single-path = deterministic bracket winner; Monte Carlo odds = simulated title frequency.';
...
const renderHeadline = (summary, predictions, backtest) => {
  ...
  addCard(headlineEl, 'Top Champion (Monte Carlo Odds)', predictions?.champion_probs?.[0]?.team || '—');
};
...
const renderPredictions = (predictions) => {
  ...
  const baseNote = predictions.notes || '';
  predictionNoteEl.textContent = baseNote
    ? `${baseNote} ${METRIC_EXPLAINER}`
    : METRIC_EXPLAINER;
};
...
const renderBacktest = (backtest) => {
  ...
  addCard(backtestCardsEl, 'RW Brier', fmt(backtest.metrics?.round_weighted_brier, 3));
  ...
  addCard(
    backtestCardsEl,
    'Estimated Kaggle Rank',
    backtest.kaggle?.estimated_rank || backtest.metrics?.estimated_kaggle_rank || 'N/A'
  );
  addCard(
    backtestCardsEl,
    'ESPN Pool Rank',
    backtest.espn_pool?.rank_position ? `#${fmt(backtest.espn_pool.rank_position, 0)}` : 'N/A',
    backtest.espn_pool?.pool_size ? `of ${fmt(backtest.espn_pool.pool_size, 0)}` : ''
  );
  const integrityLevel = backtest.integrity?.level !== undefined ? backtest.integrity.level : 'unknown';
  const integrityNote = backtest.integrity?.note || '';
  const sourceHoldout = backtest.sources?.holdout_eval ? `Holdout source: ${backtest.sources.holdout_eval}. ` : '';
  const sourceUnified = backtest.sources?.unified_backtest ? `Unified source: ${backtest.sources.unified_backtest}. ` : '';
  const baseNote = backtest.notes || '';
  backtestNoteEl.textContent = `${baseNote} Integrity level: ${integrityLevel}. ${integrityNote} ${sourceHoldout}${sourceUnified}`.trim();
};
