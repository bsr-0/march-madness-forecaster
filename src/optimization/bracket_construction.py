...
# ---------------------------------------------------------------------------
# Per-region independent construction
# ---------------------------------------------------------------------------


def _region_top_n_per_region_construction(
    by_region: Dict[str, Dict[int, str]],
    region_params: Dict[str, Tuple[Dict[str, Dict[str, float]], float]],
    late_round_probs: Dict[str, Dict[str, float]],
    late_round_risk: float,
    public_picks: Dict[str, Dict[str, float]],
    pool_size: int,
    scoring_system: Dict[str, int],
    max_outcomes: int = 100,
) -> Tuple[Dict[str, str], str, List[str], float, float]:
    """Region top-N construction with an independent scorer per region.

    Unlike ``_region_top_n_construction``, which builds ONE shared scorer
    from a single (round_probs, risk_level) pair and uses it for all 4
    regions, this function accepts ``region_params`` — a mapping from
    region name to ``(round_probs, risk_level)`` — so each region can
    use a different probability base or risk level.

    The F4 and Championship picks use a separate ``late_round_probs`` and
    ``late_round_risk`` so the caller can set a consistent late-round style
    (e.g., average risk of all 4 regions) independent of the within-region
    differentiation experiment.

    Args:
        by_region: ``{region: {seed: team_id}}`` for the 64 teams.
        region_params: ``{region: (round_probs, risk_level)}`` — one entry
            per region.  Missing regions fall back to the first available
            entry in ``region_params`` (or a 0.5 risk + the late_round_probs
            as a last resort).
        late_round_probs: round_probs dict for F4 + CHAMP picks.
        late_round_risk: risk_level for the F4 + CHAMP scorer.
        public_picks: shared opponent pick distribution.
        pool_size: pool size (controls pool_factor in the scorer).
        scoring_system: ESPN points per round.
        max_outcomes: beam width for ``_enumerate_region_outcomes``.

    Returns the same ``(picks, champion, final_four, ev, var)`` tuple as
    ``_region_top_n_construction`` / ``construct_bracket``.
    """
    # Build per-region scorers.
    # Fall back to a sane default if a region is missing from region_params.
    _fallback_rp, _fallback_risk = next(iter(region_params.values()), (late_round_probs, late_round_risk))

    all_picks: Dict[str, str] = {}
    e8_winners: Dict[str, str] = {}

    for region in _REGION_ORDER:
        rp, risk = region_params.get(region, (_fallback_rp, _fallback_risk))
        scorer = _make_ev_scorer(rp, public_picks, risk, pool_size, scoring_system)
        outcomes = _enumerate_region_outcomes(region, by_region[region], rp, scorer, max_outcomes)
        if not outcomes:
            continue
        best_picks, best_champ, _, _ = outcomes[0]
        all_picks.update(best_picks)
    ...
    # F4 + CHAMP use their own scorer.
    late_scorer = _make_ev_scorer(late_round_probs, public_picks, late_round_risk, pool_size, scoring_system)
    ...
    champion = _decide_winner(semi_ew, semi_sm, "CHAMP", set(), None, late_scorer)
    all_picks["CHAMP"] = champion

    final_four = [e8_winners.get(r, "") for r in _REGION_ORDER]
    ev, var = _compute_expected_points(all_picks, late_round_probs, scoring_system)
    return all_picks, champion, final_four, ev, var


def construct_bracket_per_region(
    seeds: Dict[str, int],
    regions: Dict[str, str],
    region_params: Dict[str, Tuple[Dict[str, Dict[str, float]], float]],
    late_round_probs: Dict[str, Dict[str, float]],
    ...
    """Public API for per-region independent bracket construction.

    Each region can use a different (round_probs, risk_level) pair, allowing
    the caller to inject a contrarian or alternative-probability-base scorer
    into a single region while keeping the other three at baseline.

    This is a parallel function to ``construct_bracket`` and does NOT modify
    it. It calls ``_region_top_n_per_region_construction`` directly.

    Args:
        seeds: ``{team_id: seed}`` for all 64–68 teams.
        regions: ``{team_id: region_name}`` for all teams.
        region_params: ``{region_name: (round_probs, risk_level)}`` for each
            of the 4 regions.  Region names must match the normalized set
            (``East``, ``West``, ``South``, ``Midwest``).
        late_round_probs: round_probs used for F4 + CHAMP.
        late_round_risk: risk_level for the F4 + CHAMP scorer.
        public_picks: opponent pick distribution.
        pool_size: pool size for the EV scorer.
        scoring_system: ESPN scoring (defaults to 10/20/40/80/160/320).
        max_outcomes: beam width for region enumeration.
    ...
    return _region_top_n_per_region_construction(
        by_region=by_region,
        region_params=region_params,
        late_round_probs=late_round_probs,
        late_round_risk=late_round_risk,
