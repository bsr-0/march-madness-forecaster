...
def run_forecast(args):
    """Run the full tournament forecasting pipeline."""
...
def run_forecast_from_manifest(args):
    """Run forecast using artifact paths from an ingestion manifest."""
...
def run_production_cmd(args):
    ...
    from ..pipeline.tournament_pipeline import DataRequirementError
...
def register(subparsers):
    # forecast (main pipeline entry point)
    forecast_parser = subparsers.add_parser("forecast", help="Run full tournament forecasting pipeline")
    ...
    add_common_pipeline_args(forecast_parser)
    forecast_parser.set_defaults(year=_default_year())
    forecast_parser.set_defaults(func=run_forecast)

    # forecast-from-manifest
    manifest_parser = subparsers.add_parser(
        "forecast-from-manifest",
        help="Run forecast using artifact paths defined in an ingestion manifest",
    )
    manifest_parser.add_argument("--manifest", required=True, help="Path to ingestion manifest JSON")
    manifest_parser.add_argument("--output", "-o", default="forecast_report.json", help="Output report JSON")
    add_common_pipeline_args(manifest_parser)
    add_manifest_override_args(manifest_parser)
    manifest_parser.set_defaults(func=run_forecast_from_manifest)
