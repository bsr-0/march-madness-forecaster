...
# _ProbabilityPredictor is inlined below (see end of file).
...
# ======================================================================
# _ProbabilityPredictor — inlined from probability_predictor.py
# (Tier 3 / 7→1 pipeline collapse. Council verdict 2026-04-14: prefer
# LOC-reduction through module-scaffolding elimination over preserving
# the per-concern file split. This class is private to the pipeline;
# no external caller ever imported it.)
# ======================================================================

from .config import BT_BLEND_WEIGHT as _BT_BLEND_WEIGHT  # noqa: E402


class _ProbabilityPredictor:
    """Inference engine that wraps a trained TournamentPipeline.

    Instantiated by TournamentPipeline and given a back-reference so it
    can read all fitted artifacts without owning or copying them.
    """
    ...
    def predict_probability_production(self, team1_id: str, team2_id: str) -> float:
        ...
        No seed overrides, no sharpening, no goto_conversion. Must be
        stable across re-runs for governance hash checks.
        """
        ...
        prob = apply_calibration(
            raw,
            p.calibration_pipeline,
            cfg.pre_calibration_clip_lo,
            cfg.pre_calibration_clip_hi,
        )
    ...
    def predict_probability_experimental(self, team1_id: str, team2_id: str) -> float:
        ...
        Includes: round-weighted calibrator, BrierPostProcessor (seed
        overrides, goto_conversion, sharpening), FLB correction, seed
        prior, consistency bonus. Only used when probability_profile ==
        "experimental".
        """
        p = self._p
        cfg = p.config
        ...
        _will_use_pp = cfg.enable_seed_overrides and getattr(p, "_brier_post_processor", None) is not None
    ...
    def _raw_fusion_probability(self, team1_id: str, team2_id: str) -> float:
        ...
        if cfg.massey_blend_weight > 0 and hasattr(p, "_external_composites") and p._external_composites:
    ...
    def _embedding_probability(
        ...
        p = self._p
        proj = p._gnn_embedding_model if model_type == "gnn" else p._transformer_embedding_model
