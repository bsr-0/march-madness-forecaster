...
class ValidatedFeatures:
    ...
    def summary(self) -> str:
        ...
        return f"ValidatedFeatures: {self.n_teams} teams, {self.feature_dim} features, integrity={report_status}"
...
class BaselineCheckpoint:
    ...
    def summary(self) -> str:
        ...
        for name, scores in self.model_scores.items():
            lines.append(f"  {name}: EV={scores.get('EV', 0):.2f}, Brier={scores.get('Brier', 0):.4f}")
        if self.best_model:
...
class SelectedModels:
    ...
    def summary(self) -> str:
        ...
        for name, scores in self.candidate_scores.items():
            lines.append(f"  {name}: EV={scores.get('mean_EV', 0):.2f}, Brier={scores.get('mean_Brier', 0):.4f}")
        return "\n".join(lines)
