...
def _select_ensemble_and_evaluate(
    ...
    if pipeline.config.enable_stacking and SKLEARN_AVAILABLE and len(trained_models) >= 2 and valid_samples >= 20:
        ...
        _tuned_logit_params = {"C": 1.0, "l1_ratio": 0}
        for name, model, _ in trained_models:
            if name == "lgb" and hasattr(model, "params"):
                ...
                _tuned_logit_params = {
                    "C": model.C,
                    "l1_ratio": model.l1_ratio if hasattr(model, "l1_ratio") else 0,
                }
            ...
            for name, model_template, _ in trained_models:
                if name == "lgb":
                    ...
                    fold_model.train(
                        ...
                        y_tr_fold,
                        feature_names=feature_names,
                        num_rounds=_tuned_xgb_rounds,
                        early_stopping_rounds=None,
                        sample_weight=w_tr_fold,
                    )
                    fold_preds = fold_model.predict(X_val_fold)
                elif name == "logit":
                    _l1r = _tuned_logit_params.get("l1_ratio", 0)
                    ...
                    fold_model.train(
                        X_tr_fold, m_tr_fold, feature_names=feature_names, num_rounds=200, sample_weight=w_tr_fold
                    )
        ...
        if np.sum(oof_mask) >= 100:
            ...
            for _mf in range(3):
                ...
                meta_oof_preds[_mf_val] = _mf_model.predict_proba(meta_X[_mf_val])[:, 1]
    ...
    if hasattr(pipeline, "_tournament_sigma_calibrator") and pipeline._tournament_sigma_calibrator is not None:
        ...
        logger.info(
            "Propagated TournamentSigmaCalibrator to baseline_model (global_sigma=%.2f)",
    ...
    if pipeline.config.enable_loyo_cv and _loyo_games_dir and LeaveOneYearOutCV is not None:
    ...
    try:
        ...
        if not complexity_audit.passed:
            logger.warning(
                "COMPLEXITY GUARD: %d effective params / %d training samples = %.1f%% (target < %.0f%%). %s",
            ...
            logger.info(
                "Complexity guard PASSED: %d effective params / %d samples = %.1f%% (target < %.0f%%).",
                complexity_audit.total_effective_params,
                train_samples,
                complexity_audit.actual_ratio * 100,
                complexity_audit.target_ratio * 100,
...
def _optimize_ensemble_weights_loyo(
    ...
    years = pipeline.config.loyo_years or [y for y in range(2015, pipeline.config.year) if y != 2020]
        ...
        for name, _, _ in trained_models:
            try:
                if name == "lgb" and LIGHTGBM_AVAILABLE:
                    ...
                    fold_preds[name] = np.clip(m.predict_proba(X_val)[:, 1], 0.01, 0.99)
                    ...
                    fold_preds[name] = np.clip(m.predict_probability(X_val), 0.01, 0.99)
                else:
    ...
    for yr in preds_by_year:
        predictions_by_year[yr] = {
            name: preds_by_year[yr].get(name, np.full(len(all_y[yr]), 0.5)) for name in model_names
        }
    ...
    logger.info(
        ...
        nested_result.n_folds,
        nested_result.n_total_samples,
        nested_result.mean_brier,
        nested_result.std_brier,
    ...
    return {
        ...
        "production_weights": {n: round(w, 3) for n, w in nested_result.production_weights.items()},
        "per_fold_brier": {str(yr): round(b, 5) for yr, b in nested_result.per_fold_brier.items()},
        "per_fold_weights": {
            str(yr): {n: round(w, 3) for n, w in ws.items()} for yr, ws in nested_result.per_fold_weights.items()
        },
    }
...
def _optimize_ensemble_weights_on_validation(
    ...
    for g in val_games:
        ...
        model_preds["gnn"].append(
            pipeline._embedding_probability(
                pipeline.gnn_embeddings.get(g.team1_id), pipeline.gnn_embeddings.get(g.team2_id), model_type="gnn"
            )
        )
        model_preds["transformer"].append(
            pipeline._embedding_probability(
                pipeline.transformer_embeddings.get(g.team1_id),
                pipeline.transformer_embeddings.get(g.team2_id),
    ...
    optimizer = EnsembleWeightOptimizer(
        step=0.05, min_weight=0.05, n_bootstrap=200, random_seed=pipeline.config.random_seed
    )
