...
class _PipelineRunner:
    ...
    def train_for_predictions(self) -> None:
        ...
        for team in teams:
            ...
            if idx % feature_report_step == 0 or idx == len(teams):
                print(
                    f"[train_for_predictions] team-features {int((idx / total_teams) * 100):>3}% ({idx}/{len(teams)})",
                    flush=True,
                )
        ...
        print(
            f"[train_for_predictions] 100% complete in {_time.time() - t0:.1f}s (teams={len(p.team_features)})",
            flush=True,
        )
    ...
    def _pre_run_validation(self) -> Dict:
        ...
        if cfg.enforce_feed_freshness:
            ...
            if missing:
                ...
                logger.warning(
                    "Stale data sources (%d): %s", len(stale), [(c.source, f"{c.staleness_hours:.0f}h") for c in stale]
                )
                checks.append(
                    {
                        "check": "data_freshness",
                        ...
                        "missing_count": 0,
                        "stale_details": [
                            {"source": c.source, "hours": c.staleness_hours, "sla": c.sla_hours} for c in stale
                        ],
                    }
                )
            else:
            ...
            checks.append(
                {
                    ...
                    "status": "warn",
                    "message": f"num_simulations={cfg.num_simulations} is low",
                }
            )
    ...
    def _run_shared_pipeline(self) -> Dict:
        ...
        p._audit_log.log_action(
            "pipeline_start",
            "system",
            {
                "year": p.config.year,
                "mode": p.config.mode,
                "num_simulations": p.config.num_simulations,
            },
        )
        ...
        adjacency, gnn_stats, baseline_stats, transformer_stats, uncertainty_stats, embedding_proj_stats = (
            self._train_models(teams, game_flows)
        )
        ...
        if market_consensus is not None:
            if getattr(p.config, "enable_vegas_cross_reference", True):
                try:
                    ...
                    model_champ = (
                        dict(bracket_sim.championship_odds) if hasattr(bracket_sim, "championship_odds") else {}
                    )
                    if model_champ:
    ...
    def _train_models(self, teams, game_flows):
        ...
        p._audit_log.log_action(
            ...
            "system",
            {
                "gnn_enabled": p.config.enable_gnn,
                "transformer_enabled": p.config.enable_transformer,
            },
        )
        if hasattr(p, "_compliance_runner"):
    ...
    def _apply_injury_reports(self, rosters: Dict) -> Dict:
        ...
        stats, positional_impacts = _dl.apply_injury_reports(
            p.config,
            rosters,
            p.positional_depth_chart,
            p.injury_severity_model,
        )
