"""Scrape NCAA tournament game results from Sports Reference.

Extends the existing TournamentSeedScraper to extract actual game
outcomes (scores, winners) from the bracket HTML on
sports-reference.com/cbb/postseason/men/{year}-ncaa.html.

The bracket pages display each round as a series of matchups with
seeds, team names, and final scores.  This scraper parses the bracket
``<div>`` to extract all 63 tournament games (R64 through NCG).
...
# Round labels based on column position in SR bracket layout
_ROUND_LABELS = {
    0: "R64",
    1: "R32",
    2: "S16",
...
class TournamentResultsScraper:
    ...
    def scrape_year(self, season: int) -> List[Dict]:
        """Scrape all tournament game results for a given season.
