...
def create_synthetic_torvik_data(num_teams: int = 362) -> List[TorVikTeam]:
    """
    Create synthetic BartTorvik-like data for testing.
    
    Generates realistic T-Rank distributions based on historical data.
    
    Args:
        num_teams: Number of teams to generate
        
    Returns:
        List of synthetic TorVikTeam objects
    """
    import random
    ...
    for i in range(num_teams):
        # Generate correlated efficiency ratings
        if i < 10:  # Elite
            adj_o = random.gauss(118, 3)
            adj_d = random.gauss(94, 3)
        elif i < 50:  # Very good
            adj_o = random.gauss(112, 4)
            adj_d = random.gauss(98, 4)
        elif i < 150:  # Above average
            adj_o = random.gauss(106, 5)
            adj_d = random.gauss(102, 5)
        elif i < 280:  # Below average
            adj_o = random.gauss(100, 5)
            adj_d = random.gauss(106, 5)
        else:  # Weak
            adj_o = random.gauss(94, 4)
            adj_d = random.gauss(110, 4)
        
        # Calculate Barthag from efficiencies
        adj_em = adj_o - adj_d
        barthag = 1 / (1 + 10 ** (-adj_em / 10))
        
        # Generate Four Factors with correlation to efficiency
        eff_factor = (adj_o - 100) / 20  # Normalize
        
        teams.append(TorVikTeam(
            ...
            effective_fg_pct=round(0.50 + eff_factor * 0.03 + random.gauss(0, 0.02), 3),
            turnover_rate=round(0.18 - eff_factor * 0.02 + random.gauss(0, 0.02), 3),
            offensive_reb_rate=round(0.30 + random.gauss(0, 0.04), 3),
            free_throw_rate=round(0.30 + random.gauss(0, 0.05), 3),
            opp_effective_fg_pct=round(0.50 - eff_factor * 0.02 + random.gauss(0, 0.02), 3),
            opp_turnover_rate=round(0.18 + random.gauss(0, 0.02), 3),
            defensive_reb_rate=round(0.70 + random.gauss(0, 0.04), 3),
            opp_free_throw_rate=round(0.30 + random.gauss(0, 0.03), 3),
            wab=round((50 - i) / 10 + random.gauss(0, 2), 2),
            wins=random.randint(max(0, 25 - i // 10), 30),
            losses=random.randint(2, min(20, 5 + i // 15)),
        ))
