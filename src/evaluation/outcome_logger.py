...
def load_predictions_from_kaggle_csv(
    ...
    Parses the ``YYYY_TeamID1_TeamID2`` format. Men's Kaggle TeamIDs
    (< 3000) are bridged to canonical internal team IDs via
    ``src.prediction.kaggle_bridge`` (the spellings-table bridge the
    production export now uses); women's Kaggle TeamIDs (>= 3000) are
    used as-is, since the women's model speaks Kaggle ids natively.

    Args:
        csv_path: Path to Kaggle submission CSV (ID, Pred columns).
        mteams_path: Path to Kaggle MTeams.csv (used to locate the data root).
        known_ids: Canonical team IDs to attempt bridging for the men's
            side (typically the teams appearing in the actual results
            being scored). If omitted, no men's rows can be bridged.
        data_root: Root directory containing a ``kaggle/`` subdirectory
            with ``MTeamSpellings.csv``. Defaults to the grandparent of
            ``mteams_path`` (i.e. ``.../data`` for ``.../data/kaggle/MTeams.csv``).
    ...
    from ..exports.kaggle import is_womens_team, parse_kaggle_id
    from ..prediction.kaggle_bridge import build_bridge, normalize_kaggle_spellings
    ...
    root = Path(data_root) if data_root else Path(mteams_path).resolve().parent.parent
    _, kag_to_canon = build_bridge(known_ids or [], normalize_kaggle_spellings(root))
    ...
    for _, row in df.iterrows():
        ...
        if is_womens_team(t1_kaggle) or is_womens_team(t2_kaggle):
            t1_canonical, t2_canonical = str(t1_kaggle), str(t2_kaggle)
        else:
            t1_canonical = kag_to_canon.get(t1_kaggle)
            t2_canonical = kag_to_canon.get(t2_kaggle)
