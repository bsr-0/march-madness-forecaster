...
**Stale entries from PR #478 (RESOLVED 2026-04-25):** the manifest previously carried 12 bracket entries hashed with a one-time git SHA (`code_version=450c54fc045b` rather than `code_version="1"`). Their cache_key shape didn't match the live-loop lookup, so `--use-cache` ran silently miss across them. Pruned in a single pass: re-primed the 3 production-mode 2026 entries (`torvik`, `f4_first_tv`, `e8_first_tv`) via `run_backtest(write_cache=True)` so they land at `code_version="1"`, then surgically removed the 12 orphan bracket entries from the manifest plus their corresponding NPZ files. The 21 GROUND_TRUTH outcomes entries (also written under the legacy code_version) were left untouched — outcomes are derived from `tournament_results_*.json`, not from sampler logic, so the code_version mismatch is cosmetic for them. Live `--use-cache` now hits all 3 production-mode 2026 entries cleanly.
...
**Scope reduction vs. original Phase 2 plan:** the initial framing ("Phase 2 adds another ~3× by hoisting opponent generation AND parallelizing the flat (year, mode) task list") had two pieces — the opponent hoist and a (year, mode) flat-dispatch parallelism. **The flat-dispatch piece was dropped** when option B was chosen for Q3: shared-per-repeat opponents are by construction incompatible with parallelizing modes independently (each mode-task would need access to the same opponent draws, which means re-broadcasting opponents across processes — losing the win). Year-level parallelism from Phase 1.A still applies; the per-mode work inside a year stays sequential. Operators who want flat (year, mode) parallelism now have a clean path: switch to `opponent_strategy="per_mode"` (each mode is independent of the others within a year), then a future Phase 3 could shard `(year, mode)` pairs across workers without correctness concerns. Not on the roadmap because year-level parallelism already saturates at `~14 workers ≈ len(BACKTEST_YEARS)` for typical hardware.

**CI integration:** `tests/test_parallel_run_backtest.py` and `tests/test_strategy_cache_phase5_equivalence.py` are now both module-marked `@pytest.mark.backtest_regression`. A new "Backtest regression / equivalence tests" step in `.github/workflows/ci.yml` runs `pytest -m "backtest_regression"` so the locks (workers-equivalence in both shared and per_mode modes, plus cache equivalence) actually execute on every push. Before this, the equivalence harness was collected by `pytest --co` for the test-count metric but never run — a no-op gate that concealed the three pre-existing bugs documented above until the data deps were installed locally.

### Validation & guardrails

| Item | Phase | Why | Scope |
|------|-------|-----|-------|
| ~~Drift-guard test for Oracle output~~ | ~~1b10 follow-up~~ | ~~`oracle_sweep_t3_years()` ships its data into `summary["tiers"]["T3"]["oracle"]` but nothing currently fails CI if `ranker_gap_espn_pts` regresses.~~ — **DONE 2026-04-25** (`tests/test_oracle_drift_guard.py`, 51 parametrized anchors). Locks per-(year, strategy) gap / F4-hits / champ-hit for the 12 cells (3 strategies × 4 years from the ledger), plus 12 strategy-level aggregate anchors, plus a direction-only regime check (chaos 2023 ≥ 500 in some mode; chalk 2025 < 500 in every mode). Catches drift in oracle math, committed bracket artifacts, ground-truth ingestion, and ESPN scoring vector. |
